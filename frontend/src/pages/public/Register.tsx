...
export default function Register() {
  ...
  useEffect(() => {
    const fetchBranches = async () => {
      try {
        const response = await fetch('http://localhost:8000/api/v1/auth/public/branches');
  ...
  const sendOTP = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/v1/auth/send-verification-otp', {
        method: 'POST',
  ...
  const verifyOTP = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/v1/auth/verify-email-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
