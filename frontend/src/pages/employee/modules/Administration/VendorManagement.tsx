...
export default function VendorManagement() {
  ...
  const fetchVendors = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8000/api/v1/auth/employee/administration/vendors', {
  ...
  const handleDeleteVendor = async (name: string) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/v1/auth/employee/administration/vendors/${vendorId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
