...
import { 
  ...
  TrendingUp, ClipboardList, MessageSquare, ShieldCheck, 
  Settings, LogOut
} from 'lucide-react';

export default function BranchManagerSidebar({ user }: { user: any }) {
  ...
  return <SidebarTemplate title="Branch Management" role="Branch Manager" icon={<MapPin />} items={menuItems} user={user} isActive={isActive} />;
}

function SidebarTemplate({ title, role, icon, items, user, isActive }: any) {
  const handleLogout = () => {
    ...
    <nav className="fixed left-0 top-0 h-full w-64 bg-slate-950 border-r border-slate-800 flex flex-col z-50">
      <div className="p-6 flex items-center gap-3 mb-2">
        ...
        <div className="overflow-hidden">
           ...
           <div className="text-[10px] text-indigo-400 font-bold tracking-widest uppercase truncate">{title}</div>
        </div>
      </div>

      <div className="flex-1 overflow-y-auto px-3 space-y-4 pb-20 custom-scrollbar">
        <div className="px-4 py-2 text-[10px] font-bold text-slate-500 uppercase tracking-widest">{role} Desk</div>
