...
import { 
  LayoutDashboard, MapPin, Target, CheckSquare, Users, 
  Activity, ClipboardList, MessageSquare, 
  Settings, LogOut
} from 'lucide-react';

export default function AssistantManagerSidebar({ user }: { user: any }) {
  const location = useLocation();
  const isActive = (path: string) => location.pathname === path;

  const menuItems = [
    { to: '/employee', icon: <LayoutDashboard />, label: 'Operational Dashboard' },
    { to: '/employee/performance', icon: <Activity />, label: 'Station Monitor' },
    { to: '/employee/approvals', icon: <CheckSquare />, label: 'Urgent Approvals' },
    { to: '/employee/staff', icon: <Users />, label: 'Staff Support' },
    { to: '/employee/inventory', icon: <Target />, label: 'Inventory Desk' },
    { to: '/employee/reports', icon: <ClipboardList />, label: 'Daily Reports' },
    { to: '/employee/escalations', icon: <MessageSquare />, label: 'Customer Resolution' },
    { to: '/employee/settings', icon: <Settings />, label: 'Preferences' }
  ];
    ...
    <nav className="fixed left-0 top-0 h-full w-64 bg-slate-950 border-r border-slate-800 flex flex-col z-50">
      <div className="p-6 flex items-center gap-3 mb-2">
        ...
        <div className="overflow-hidden">
           ...
           <div className="text-[10px] text-indigo-400 font-bold tracking-widest uppercase truncate">Asst Manager Desk</div>
        </div>
      </div>

      <div className="flex-1 overflow-y-auto px-3 space-y-4 pb-20 custom-scrollbar">
        <div className="px-4 py-2 text-[10px] font-bold text-slate-500 uppercase tracking-widest">Workspace</div>
