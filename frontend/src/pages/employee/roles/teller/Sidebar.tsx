...
import { 
  LayoutDashboard, Wallet, Banknote, CreditCard, ArrowLeftRight, 
  Receipt, History, PieChart, Clock, LogOut
} from 'lucide-react';

export default function TellerSidebar({ user }: { user: any }) {
  const location = useLocation();
  const isActive = (path: string) => location.pathname === path;

  const menuItems = [
    { to: '/employee', icon: <LayoutDashboard />, label: 'Teller Console' },
    { to: '/employee/deposit', icon: <Banknote />, label: 'Customer Deposit' },
    { to: '/employee/withdrawal', icon: <CreditCard />, label: 'Customer Withdrawal' },
    { to: '/employee/transfer', icon: <ArrowLeftRight />, label: 'Account Transfer' },
    { to: '/employee/forex', icon: <CreditCard />, label: 'Foreign Exchange' },
    { to: '/employee/utility', icon: <Receipt />, label: 'Utility Payments' },
    { to: '/employee/logs', icon: <History />, label: 'My Transactions' },
    { to: '/employee/balance', icon: <PieChart />, label: 'Drawer Balance' },
    { to: '/employee/closing', icon: <Clock />, label: 'Shift Closing' }
  ];
    ...
    <nav className="fixed left-0 top-0 h-full w-64 bg-slate-950 border-r border-slate-800 flex flex-col z-50">
      <div className="p-6 flex items-center gap-3 mb-2">
        ...
        <div className="overflow-hidden">
           ...
           <div className="text-[10px] text-indigo-400 font-bold tracking-widest uppercase truncate">Teller Workspace</div>
        </div>
      </div>

      <div className="flex-1 overflow-y-auto px-3 space-y-4 pb-20 custom-scrollbar">
        <div className="px-4 py-2 text-[10px] font-bold text-slate-500 uppercase tracking-widest">Transaction Desk</div>
