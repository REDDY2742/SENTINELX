...
import { 
  LayoutDashboard, Wallet, Banknote, CreditCard, 
  History, PieChart, Clock, ShieldCheck, LogOut
} from 'lucide-react';

export default function CashierSidebar({ user }: { user: any }) {
  const location = useLocation();
  const isActive = (path: string) => location.pathname === path;

  const menuItems = [
    { to: '/employee', icon: <LayoutDashboard />, label: 'Vault Dashboard' },
    { to: '/employee/deposit', icon: <Banknote />, label: 'Bulk Deposit' },
    { to: '/employee/withdrawal', icon: <CreditCard />, label: 'Vault Withdrawal' },
    { to: '/employee/petty-cash', icon: <Wallet />, label: 'Petty Cash' },
    { to: '/employee/coins', icon: <PieChart />, label: 'Coin Management' },
    { to: '/employee/logs', icon: <History />, label: 'High-Value Journal' },
    { to: '/employee/balance', icon: <ShieldCheck />, label: 'Vault Reconciliation' },
    { to: '/employee/closing', icon: <Clock />, label: 'EOD Settlement' }
  ];
    ...
    <nav className="fixed left-0 top-0 h-full w-64 bg-slate-950 border-r border-slate-800 flex flex-col z-50">
      <div className="p-6 flex items-center gap-3 mb-2">
        ...
        <div className="overflow-hidden">
           ...
           <div className="text-[10px] text-indigo-400 font-bold tracking-widest uppercase truncate">Cashier Desk</div>
        </div>
      </div>

      <div className="flex-1 overflow-y-auto px-3 space-y-4 pb-20 custom-scrollbar">
        <div className="px-4 py-2 text-[10px] font-bold text-slate-500 uppercase tracking-widest">Vault Desk</div>
